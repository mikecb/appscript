// Turn the results of a google form into a google doc that is emailed to specificed addresses. Make a form, send responses to spreadsheet, open that spreadsheet>Tools>Script Editor. 
...
function EmailGoogleFormData(e) {
  ...
  if (!e) {
    throw new Error("Select Initialize from the Run menu, then accept oauth permissions.");
  }
  ...
  try {
    ...
    if (MailApp.getRemainingDailyQuota() > 0) {
      ...
      for (var keys in cols) {
        ...
        body.appendParagraph(key).setHeading(DocumentApp.ParagraphHeading.HEADING5);
      ...
      MailApp.sendEmail({to: email, subject: subject, body: ' ' + doc.getUrl(), noReply: true,});
    }
  } catch (error) {
    Logger.log(error.toString());
    admin = ""; //email address for error logs to be sent to.
    MailApp.sendEmail({to: admin, subject: "Error in script ____", body: error.toString() + "\n" + Logger.getLog(), noReply: true,});
  }
}
